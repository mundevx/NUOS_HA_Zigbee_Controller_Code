...
void DaliCommands::set_rgbwaf_ctrl() {
    uint8_t data = 0b00111111;  // Setting channels
    send_command_special(SET_DTR0, data);
...
// void DaliCommands::set_temp_primary_dim(uint8_t addr, uint8_t mode){
//     daliCore.sendCommandPublic(SET_DTR0, mode);
//     daliCore.sendCommandPublic(SHORT_POWER | ((addr << 1) & 0x7e) | 0x01, SET_TEMP_PRIMARY_DIM);
//     daliCore.sendCommandPublic(SHORT_POWER | ((addr << 1) & 0x7e) | 0x01, SET_TEMP_PRIMARY_DIM);
// }
...
void DaliCommands::set_broadcast_fade_time(uint8_t time)
{
    ...
    //delay(DELAY_COMMAND_SEND);
    send_command_standard(SET_FADE_TIME, BROADCAST_DP);  
    //daliCore.sendCommandPublic(BROADCAST_DP, SET_FADE_TIME);
    //daliCore.sendCommandPublic(BROADCAST_DP, time);            
}
...
void DaliCommands::set_broadcast_level(uint8_t value){
    ...
    //     output = 254;
    // }else{
    //     output = (uint8_t)(((value + 130) * 254.0) / (240 + 130) + 0.5);
    //     if(output > 240 ) output = 254;
    // }
    // printf("In:%d Out:%d\n", value, output);
    // daliCore.sendCommandPublic(0x80 | (0xff << 1), output);
...
void DaliCommands::go_to_scene(uint8_t addr, uint8_t scene)
{
    ...
    //daliCore.sendCommandPublic(ENABLE_DEVICE_TYPE, 0x08);
