...
String HEAD_TITLE ="<head> <meta charset=\"UTF-8\"> <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"> <title>Select Scene Devices (NUOS)</title> </head>";
#elif(USE_NUOS_ZB_DEVICE_TYPE == DEVICE_CCT_DALI || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_CCT_DALI_CUSTOM || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_RGB_DALI || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_RGB_DMX || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_RGB_CUSTOM || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_GROUP_DALI)
String HEAD_TITLE ="<head> <meta charset=\"UTF-8\"> <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"> <title>DALI Group Devices</title> </head>";
#elif(USE_NUOS_ZB_DEVICE_TYPE == DEVICE_SCENE_DALI)
String HEAD_TITLE ="<head> <meta charset=\"UTF-8\"> <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"> <title>DALI Scene Switch</title> </head>";
#endif
/********************STYLE**********************/
String STYLE_START = "<style>";
// String STYLE ="body { display: flex; flex-direction: column; align-items: center; justify-content: flex-start; min-height: 100vh; margin: 0; padding-top: 20px; } .grid-container { display: grid; grid-template-columns: repeat(2, 1fr); grid-template-rows: auto auto; row-gap: 30px; column-gap: 60px; width: 100%; max-width: auto; } .form-container { background-color: white; padding: 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); width: 560px; } .form { display: flex; flex-direction: column; } .form-row { display: flex; justify-content: space-between; margin-bottom: 10px; } .form-row input[type=\"number\"] { flex: 1; margin: 0 5px; padding: 10px; border: 1px solid #ddd; border-radius: 4px; } .form-group { margin-bottom: 5px; } .h1 { text-align: center; color: #333; margin-bottom: 15px; } .h2 { background: linear-gradient(to right, #6dd5ed, #2193b0); color: white; padding: 10px 10px; border-radius: 8px 8px 0 0; text-align: center; margin: -20px -20px 20px -20px; } .h3 { font-size: 12px; font-style: italic; } label { display: block; margin-bottom: 5px; } .input[type=\"number\"] { width: 100%; padding: 10px 15px; margin: 8px 0; box-sizing: border-box; border: 2px solid #FF3D00; border-radius: 4px; background-color: #FAFAFA; transition: border-color 0.3s, box-shadow 0.3s; box-shadow: 0 0px 2px rgba(0, 50, 0, 0.6); } .input::placeholder { color: #BDBDBD; opacity: 1; } input:hover { border-color: #66afe9; } input:focus { border-color: #4d90fe; box-shadow: 0 0 5px rgba(81, 203, 238, 1); } .submit-button { background: linear-gradient(to right, #2193b0, #2193b0); color: white; border: none; padding: 10px 15px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; border-radius: 4px; cursor: pointer; width: 100%; transition-duration: 0.4s; box-shadow: 0 2px 4px rgba(0, 150, 150, 0.6); } .stylish-line { border: 0; height: 1px; background-image: linear-gradient(to right, #ffffff, #ffffff); margin: 20px 0; box-shadow: 0 1px 4px rgba(0, 0, 100, 0.6); } .tabs-container { display: flex; background-color: #fff; border-radius: 8px; overflow: hidden; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); } .tab { flex: 1; padding: 15px; text-align: center; cursor: pointer; transition: background-color 0.3s, color 0.3s; } .tab:hover { background-color: #f0f0f0; } .tab.active { background-color: #3498db; color: #fff; } .stylish-button { background-color: #2196F3; border: none; color: white; padding: 10px 15px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; margin: 4px 2px; cursor: pointer; border-radius: 4px; transition-duration: 0.4s; } .stylish-button:hover { background-color: white; color: #2196F3; border: 2px solid #2196F3; } table { border-collapse: collapse; width: 100%; margin-top: 20px; } th, td { border: 1px solid #ddd; padding: 8px; text-align: left; } th { background-color: #f2f2f2; } tr:nth-child(even) { background-color: #f9f9f9; } .card1 { display: flex; flex-wrap: wrap; justify-content: space-between; } .container1 { flex-basis: calc(48% - 20px); margin-bottom: 20px; box-sizing: border-box; margin-right: 20px; } .container { display: flex; flex-direction: column; align-items: center; width: 100%; } .card { background-color: #fff; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); padding: 20px; margin-top: 20px; display: none; } .tabs-container { display: flex; background-color: #fff; border-radius: 8px; overflow: hidden; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); } .card.active { display: block; } .wifi-network { display: flex; align-items: center; margin-bottom: 10px; padding: 10px; border: 1px solid #ccc; border-radius: 8px; } .wifi-range-icon { width: 24px; height: 24px; margin-left: 10px; background-color: #3498db; border-radius: 50%; }";
...
String STYLE ="body { display: flex; flex-direction: column; align-items: center; justify-content: flex-start; min-height: 100vh; margin: 0; padding-top: 20px; } .grid-container { display: grid; grid-template-columns: repeat(2, 1fr); grid-template-rows: auto auto; row-gap: 30px; column-gap: 60px; width: 100%; max-width: auto; } .form-container { background-color: white; padding: 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); width: 560px; } .form { display: flex; flex-direction: column; } .form-row { display: flex; justify-content: space-between; margin-bottom: 10px; } .form-row input[type=\"number\"] { flex: 1; margin: 0 5px; padding: 10px; border: 1px solid #ddd; border-radius: 4px; } .form-group { margin-bottom: 5px; } .h1 { text-align: center; color: #333; margin-bottom: 15px; } .h2 { background: linear-gradient(to right, #6dd5ed, #2193b0); color: white; padding: 10px 10px; border-radius: 8px 8px 0 0; text-align: center; margin: -20px -20px 20px -20px; } .h3 { font-size: 12px; font-style: italic; } label { display: block; margin-bottom: 5px; } input[type=\"number\"] { width: 100%; padding: 10px 15px; margin: 8px 0; box-sizing: border-box; border: 2px solid #FF3D00; border-radius: 4px; background-color: #FAFAFA; transition: border-color 0.3s, box-shadow 0.3s; box-shadow: 0 0px 2px rgba(0, 50, 0, 0.6); } input::placeholder { color: #BDBDBD; opacity: 1; } input { padding: 10px; border: 1px solid #ccc; border-radius: 5px; outline: none; font-size: 16px; transition: border-color 0.3s; } input:hover { border-color: #66afe9; } input:focus { border-color: #4d90fe; box-shadow: 0 0 5px rgba(81, 203, 238, 1); } .submit-button { background: linear-gradient(to right, #2193b0, #2193b0); color: white; border: none; padding: 10px 15px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; border-radius: 4px; cursor: pointer; width: 100%; transition-duration: 0.4s; box-shadow: 0 2px 4px rgba(0, 150, 150, 0.6); } .stylish-line { border: 0; height: 1px; background-image: linear-gradient(to right, #ffffff, #ffffff); margin: 20px 0; box-shadow: 0 1px 4px rgba(0, 0, 100, 0.6); } .tabs-container { display: flex; background-color: #fff; border-radius: 8px; overflow: hidden; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); } .tab { flex: 1; padding: 15px; text-align: center; cursor: pointer; transition: background-color 0.3s, color 0.3s; } .tab:hover { background-color: #f0f0f0; } .tab.active { background-color: #3498db; color: #fff; } .stylish-button { background-color: #2196F3; border: none; color: white; padding: 10px 15px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; margin: 4px 2px; cursor: pointer; border-radius: 4px; transition-duration: 0.4s; } .stylish-button:hover { background-color: white; color: #2196F3; border: 2px solid #2196F3; } table { border-collapse: collapse; width: 100%; margin-top: 20px; } th, td { border: 1px solid #ddd; padding: 8px; text-align: left; } th { background-color: #f2f2f2; } tr:nth-child(even) { background-color: #f9f9f9; } .container { display: flex; flex-direction: column; align-items: center; width: 100%; } .card { background-color: #fff; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); padding: 20px; margin-top: 20px; display: none; } .card.active { display: block; }";
#elif(USE_NUOS_ZB_DEVICE_TYPE == DEVICE_SCENE_DALI)
String STYLE ="body { display: flex; flex-direction: column; align-items: center; justify-content: flex-start; min-height: 100vh; margin: 0; padding-top: 20px; } .h2 { background: linear-gradient(to right, #6dd5ed, #2193b0); color: white; padding: 10px 10px; border-radius: 8px 8px 0 0; text-align: center; margin: -20px -20px 20px -20px; } label { display: block; margin-bottom: 5px; } .stylish-button { background-color: #2196F3; border: none; color: white; padding: 12px 0; text-align: center; text-decoration: none; display: block; font-size: 16px; margin: 4px 0; cursor: pointer; border-radius: 4px; transition-duration: 0.4s; width: 100%; box-sizing: border-box; }";
#else
String STYLE = "body { display: flex; flex-direction: column; align-items: center; justify-content: flex-start; min-height: 100vh; margin: 0; padding-top: 20px; } .grid-container { display: grid; grid-template-columns: repeat(2, 1fr); grid-template-rows: auto auto; row-gap: 30px; column-gap: 60px; width: 100%; max-width: auto; } .form-container { background-color: white; padding: 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); width: 560px; } .form { display: flex; flex-direction: column; } .form-row { display: flex; justify-content: space-between; margin-bottom: 10px; } .form-row input[type=\"number\"] { flex: 1; margin: 0 5px; padding: 10px; border: 1px solid #ddd; border-radius: 4px; } .form-group { margin-bottom: 5px; } .h1 { text-align: center; color: #333; margin-bottom: 15px; } .h2 { background: linear-gradient(to right, #6dd5ed, #2193b0); color: white; padding: 10px 10px; border-radius: 8px 8px 0 0; text-align: center; margin: -20px -20px 20px -20px; } .h3 { font-size: 12px; font-style: italic; } label { display: block; margin-bottom: 5px; } .input[type=\"number\"] { width: 100%; padding: 10px 15px; margin: 8px 0; box-sizing: border-box; border: 2px solid #FF3D00; border-radius: 4px; background-color: #FAFAFA; transition: border-color 0.3s, box-shadow 0.3s; box-shadow: 0 0px 2px rgba(0, 50, 0, 0.6); } .input::placeholder { color: #BDBDBD; opacity: 1; } input { padding: 10px; border: 1px solid #ccc; border-radius: 5px; outline: none; font-size: 16px; transition: border-color 0.3s; } input:hover { border-color: #66afe9; } input:focus { border-color: #4d90fe; box-shadow: 0 0 5px rgba(81, 203, 238, 1); } .submit-button { background: linear-gradient(to right, #2193b0, #2193b0); color: white; border: none; padding: 10px 15px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; border-radius: 4px; cursor: pointer; width: 100%; transition-duration: 0.4s; box-shadow: 0 2px 4px rgba(0, 150, 150, 0.6); } .stylish-line { border: 0; height: 1px; background-image: linear-gradient(to right, #ffffff, #ffffff); margin: 20px 0; box-shadow: 0 1px 4px rgba(0, 0, 100, 0.6); } .tabs-container { display: flex; background-color: #fff; border-radius: 8px; overflow: hidden; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); } .tab { flex: 1; padding: 15px; text-align: center; cursor: pointer; transition: background-color 0.3s, color 0.3s; } .tab:hover { background-color: #f0f0f0; } .tab.active { background-color: #3498db; color: #fff; } .stylish-button { background-color: #2196F3; border: none; color: white; padding: 10px 15px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; margin: 4px 2px; cursor: pointer; border-radius: 4px; transition-duration: 0.4s; } .stylish-button:hover { background-color: white; color: #2196F3; border: 2px solid #2196F3; } input[type=\"number\"] { width: 50px; height: 30px; text-align: center; } table { border-collapse: collapse; width: 100%; margin-top: 20px; } th, td { border: 1px solid #ddd; padding: 8px; text-align: left; } th { background-color: #f2f2f2; } tr:nth-child(even) { background-color: #f9f9f9; } .card1 { display: flex; flex-wrap: wrap; justify-content: space-between; } .container1 { flex-basis: calc(48% - 20px); margin-bottom: 20px; box-sizing: border-box; margin-right: 20px; } .container { display: flex; flex-direction: column; align-items: center; width: 100%; } .card { background-color: #fff; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); padding: 20px; margin-top: 20px; display: none; } .tabs-container { display: flex; background-color: #fff; border-radius: 8px; overflow: hidden; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); } .card.active { display: block; } .wifi-network { display: flex; align-items: center; margin-bottom: 10px; padding: 10px; border: 1px solid #ccc; border-radius: 8px; } .wifi-range-icon { width: 24px; height: 24px; margin-left: 10px; background-color: #3498db; border-radius: 50%; }";
#endif
...
#endif
#else
String STYLE_SCENE_SWITCH =".form { display: flex; flex-direction: column; } .card { background: #fff; border-radius: 18px; box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.1); padding: 2.5em 2em 2em 2em; max-width: 410px; width: 100; margin: 2em; display: flex; flex-direction: column; align-items: center; } p { color: #64748b; font-size: 1em; margin-bottom: 0.1em; text-align: center; } input[type=\"number\"] { width: 100%; padding: 10px 15px; margin: 8px 0; box-sizing: border-box; border: 2px solid #FF3D00; border-radius: 4px; background-color: #FAFAFA; transition: border-color 0.3s, box-shadow 0.3s; box-shadow: 0 0px 2px rgba(0, 50, 0, 0.6); } input[type=\"number\"], select { width: 100%; padding: 0.7em 0.8em; border: 1.5px solid #cbd5e1; border-radius: 8px; background: #f1f5f9; font-size: 1.08em; margin-top: 0.15em; transition: border-color 0.2s; outline: none; } input[type=\"number\"]:focus, select:focus { border-color: #3b82f6; background: #e0e7ff; } .row { display: flex; gap: 1em; } .row > div { flex: 1; } button:hover { background: linear-gradient(90deg, #6366f1 0, #3b82f6 100); transform: translateY(-2px) scale(1.02); } #result { margin-top: 1.5em; background: #f1f5f9; color: #475569; padding: 1em; border-radius: 8px; min-height: 1.5em; font-size: 0.98em; word-break: break-all; width: 100; text-align: left; box-sizing: border-box; display: none; } @media (max-width: 500px) { .card { padding: 1.2em 0.5em 1.2em 0.5em; max-width: 98vw; } }";
String STYLE_SCENE_SWITCH_POPUP =".popup { display: none; position: fixed; left: 50%; top: 50%; transform: translate(-50%, -50%); background: #fff; border: 1px solid #888; z-index: 100; padding: 20px; max-width: 90vw; overflow-y: auto; } .overlay { display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.4); z-index: 99; } .toggle-btn { width: 40px; height: 40px; margin: 2px; font-size: 16px; cursor: pointer; border: 1px solid #ccc; background: #f0f0f0; } .selected { background: #4CAF50; color: #fff; border-color: #388E3C; } #toggleGrid { max-height: 60vh; overflow-y: auto; }";
...
String STYLE_DIALOG_BOX ="#overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5); display: none; z-index: 1; } #confirmationDialog { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); padding: 20px; width: 300px; background-color: #fff; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2); z-index: 2; display: none; text-align: center; animation: fadeIn 0.3s; } @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } } #confirmationDialog p { font-size: 16px; color: #333; margin-bottom: 20px; } .dialog-btn { padding: 10px 20px; border: none; border-radius: 4px; font-size: 14px; cursor: pointer; margin: 0 10px; } .dialog-btn-yes { background-color: #28a745; color: #fff; } .dialog-btn-yes:hover { background-color: #218838; } .dialog-btn-no { background-color: #dc3545; color: #fff; } .dialog-btn-no:hover { background-color: #c82333; }";
String STYLE_BUTTON_SPINNER =".spinner-icon { width: 16px; height: 16px; margin-left: 8px; vertical-align: middle; animation: spin 0.8s linear infinite; } .spinner-icon.hidden { display: none; } button:disabled { opacity: 0.6; cursor: not-allowed; }";
String STYLE_IP_ADDR =".ip-input-group { display: flex; justify-content: center; align-items: center; gap: 0.5rem; margin-bottom: 0.1rem; } .ip-segment { width: 50px; padding: 0.5rem; font-size: 1em; text-align: center; border: 1px solid #ddd; border-radius: 4px; transition: border-color 0.3s ease; } .ip-segment:focus { outline: none; border-color: #5b9bd5; } .dot { font-size: 1.2em; color: #333; vertical-align: middle; }";
...
String TAB_DISPLAY ="<div class=\"tabs-container\"> <div class=\"tab\" onclick=\"openTab(event, 'Tab2')\">Save Dali IDs</div> <div class=\"tab\" onclick=\"openTab(event, 'Tab3')\">Show Dali Addresses</div> </div>";
#else
...
String TAB2_START = "<div id=\"Tab2\" class=\"card\"> <div class=\"container\"> <h1>Select Your AC Model</h1>";
String TAB2_BODY ="<form name=\"acForm\" action=\"/action\" method=\"post\" onsubmit=\"return false;\">";
String TAB2_SELECT ="<select id=\"ac-model\" name=\"ac_model\"> <option value=\"\">--Select a model--</option> <option value=\"RC5\">RC5</option> <option value=\"RC6\">RC6</option> <option value=\"NEC\">NEC</option> <option value=\"SONY\">SONY</option> <option value=\"PANASONIC\">PANASONIC</option> <option value=\"JVC\">JVC</option> <option value=\"SAMSUNG\">SAMSUNG</option> <option value=\"WHYNTER\">WHYNTER</option> <option value=\"AIWA_RC_T501\">AIWA_RC_T501</option> <option value=\"LG\">LG</option> <option value=\"SANYO\">SANYO</option> <option value=\"MITSUBISHI\">MITSUBISHI</option> <option value=\"DISH\">DISH</option> <option value=\"SHARP\">SHARP</option> <option value=\"COOLIX\">COOLIX</option> <option value=\"DAIKIN\">DAIKIN</option> <option value=\"DENOM\">DENOM</option> <option value=\"KELVINATOR\">KELVINATOR</option> <option value=\"SHERWOOD\">SHERWOOD</option> <option value=\"MITSUBISHI_AC\">MITSUBISHI_AC</option> <option value=\"RCMM\">RCMM</option> <option value=\"SANYO_LC7461\">SANYO_LC7461</option> <option value=\"RC5X\">RC5X</option> <option value=\"GREE\">GREE</option> <option value=\"PRONTO\">PRONTO</option> <option value=\"NEC_LIKE\">NEC_LIKE</option> <option value=\"AGRO\">AGRO</option> <option value=\"TROTEC\">TROTEC</option> <option value=\"NIKAI\">NIKAI</option> <option value=\"RAW\">RAW</option> <option value=\"GLOBALCACHE\">GLOBALCACHE</option> <option value=\"TOSHIBA_AC\">TOSHIBA</option> <option value=\"FUJITSU_AC\">FUJITSU</option> <option value=\"MIDEA\">MIDEA</option> <option value=\"MAGIQUEST\">MAGIQUEST</option> <option value=\"LASERTAG\">LASERTAG</option> <option value=\"CARRIER_AC\">CARRIER_AC</option> <option value=\"HAIER_AC\">HAIER_AC</option> <option value=\"MITSUBISHI2\">MITSUBISHI2</option> <option value=\"HITACHI_AC\">HITACHI_AC</option> <option value=\"HITACHI_AC1\">HITACHI_AC1</option> <option value=\"HITACHI_AC2\">HITACHI_AC2</option> <option value=\"GICABLE\">GICABLE</option> <option value=\"HAIER_AC_YRW02\">HAIER_AC_YRW02</option> <option value=\"WHIRLPOOL_AC\">WHIRLPOOL_AC</option> <option value=\"SAMSUNG_AC\">SAMSUNG_AC</option> <option value=\"LUTRON\">LUTRON</option> <option value=\"ELECTRA_AC\">ELECTRA_AC</option> <option value=\"PANASONIC_AC\">PANASONIC</option> <option value=\"PIONEER\">PIONEER</option> <option value=\"LG2\">LG2</option> <option value=\"MWM\">MWM</option> <option value=\"DAIKIN2\">DAIKIN2</option> <option value=\"VESTEL_AC\">VESTEL_AC</option> <option value=\"TECO\">TECO</option> <option value=\"SAMSUNG36\">SAMSUNG36</option> <option value=\"TCL112AC\">TCL112AC</option> <option value=\"LEGOPF\">LEGOPF</option> <option value=\"MITSUBISHI_HEAVY_88\">MITSUBISHI_HEAVY_88</option> <option value=\"MITSUBISHI_HEAVY_152\">MITSUBISHI_HEAVY_152</option> <option value=\"DAIKIN216\">DAIKIN216</option> <option value=\"SHARP_AC\">SHARP_AC</option> <option value=\"GOODWEATHER\">GOODWEATHER</option> <option value=\"INAX\">INAX</option> <option value=\"DAIKIN160\">DAIKIN160</option> <option value=\"NEOCLIMA\">NEOCLIMA</option> <option value=\"DAIKIN176\">DAIKIN176</option> <option value=\"DAIKIN128\">DAIKIN128</option> <option value=\"AMCOR\">AMCOR</option> <option value=\"DAIKIN152\">DAIKIN152</option> <option value=\"MITSUBISHI136\">MITSUBISHI136</option> <option value=\"MITSUBISHI112\">MITSUBISHI112</option> <option value=\"HITACHI_AC424\">HITACHI_AC424</option> <option value=\"SONY_38K\">SONY_38K</option> <option value=\"EPSON\">EPSON</option> <option value=\"SYMPHONY\">SYMPHONY</option> <option value=\"HITACHI_AC3\">HITACHI_AC3</option> <option value=\"DAIKIN64\">DAIKIN64</option> <option value=\"AIRWELL\">AIRWELL</option> <option value=\"DELONGHI_AC\">DELONGHI_AC</option> <option value=\"DOSHISHA\">DOSHISHA</option> <option value=\"MULTIBRACKETS\">MULTIBRACKETS</option> <option value=\"CARRIER_AC40\">CARRIER_AC40</option> <option value=\"CARRIER_AC64\">CARRIER_AC64</option> <option value=\"HITACHI_AC344\">HITACHI_AC344</option> <option value=\"CORONA_AC\">CORONA_AC</option> <option value=\"MIDEA24\">MIDEA24</option> <option value=\"ZEPEAL\">ZEPEAL</option> <option value=\"SANYO_AC\">SANYO_AC</option> <option value=\"VOLTAS\">VOLTAS</option> <option value=\"METZ\">METZ</option> <option value=\"TRANSCOLD\">TRANSCOLD</option> <option value=\"TECHNIBEL_AC\">TECHNIBEL</option> <option value=\"MIRAGE\">MIRAGE</option> <option value=\"ELITESCREENS\">ELITESCREENS</option> <option value=\"PANASONIC_AC32\">PANASONIC_AC32</option> <option value=\"MILESTAG2\">MILESTAG2</option> <option value=\"ECOCLIM\">ECOCLIM</option> <option value=\"XMP\">XMP</option> <option value=\"TRUMA\">TRUMA</option> <option value=\"HAIER_AC176\">HAIER_AC176</option> <option value=\"TEKNOPOINT\">TEKNOPOINT</option> <option value=\"KELON\">KELON</option> <option value=\"TROTEC_3550\">TROTEC_3550</option> <option value=\"SANYO_AC88\">SANYO_AC88</option> <option value=\"BOSE\">BOSE</option> <option value=\"ARRIS\">ARRIS</option> <option value=\"RHOSS\">RHOSS</option> <option value=\"AIRTON\">AIRTON</option> <option value=\"COOLIX48\">COOLIX48</option> <option value=\"HITACHI_AC264\">HITACHI_AC264</option> <option value=\"KELON168\">KELON168</option> <option value=\"HITACHI_AC296\">HITACHI_AC296</option> <option value=\"DAIKIN200\">DAIKIN200</option> <option value=\"HAIER_AC160\">HAIER_AC160</option> <option value=\"CARRIER_AC128\">CARRIER_AC128</option> <option value=\"TOTO\">BOSE</option> <option value=\"CLIMABUTLER\">CLIMABUTLER</option> <option value=\"TCL96AC\">TCL96AC</option> <option value=\"BOSCH144\">BOSCH144</option> <option value=\"SANYO_AC152\">SANYO_AC152</option> <option value=\"DAIKIN312\">DAIKIN312</option> <option value=\"GORENJE\">GORENJE</option> <option value=\"WOWWEE\">WOWWEE</option> <option value=\"CARRIER_AC84\">CARRIER_AC84</option> <option value=\"YORK\">YORK</option> <option value=\"BLUESTARHEAVY\">BLUESTARHEAVY</option> </select>";
String TAB2_BUTTON ="<div style=\"display: flex; justify-content: space-between; width: 100%;\"> <button type=\"button\" style=\"background-color: blue; color: white;\" onclick=\"validateAcModel('3')\">Try</button> <button type=\"button\" style=\"background-color: orange; color: white; margin: 0 10px;\" onclick=\"validateAcModel('2')\">Save</button> <button type=\"button\" style=\"background-color: red; color: white;\" onclick=\"validateAcModel('4')\">Disable WiFi</button> </div>";
String TAB2_END = " </form></div></div>";
#elif(USE_NUOS_ZB_DEVICE_TYPE == DEVICE_SCENE_SWITCH || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_GROUP_SWITCH || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_WIRELESS_REMOTE_SWITCH)
String TAB1_START ="<div id=\"Tab1\" data-tab=\"Tab1\" class=\"card\"> <div class=\"card1\">";
...
String  DEVICE1_FORM = "]; function updateValue1(index, value) { values1[index] = Number(value); const row = Math.floor(index / 5) + 1; const col = (index % 5) + 1; const input = document.getElementById(`input0_${row}${col}`); if (input) input.value = value; } </script> <table> <tbody> <script> let index1 = 0; for (let row = 1; row <= 2; row++) { document.write(\"<tr>\"); for (let col = 1; col <= 5; col++) { document.write(`<td><input type=\"number\" id=\"input0_${row}${col}\" name=\"input${row}_${col}\" value=\"${values1[index1]}\" oninput=\"updateValue1(${index1}, this.value)\"></td>`); index1++; } document.write(\"</tr>\"); } </script> </tbody> </table> <br> <button type=\"button\" class=\"submit-button\" onclick=\"submitForm('10', 0)\">Save DALI IDs</button> <br><hr class=\"stylish-line\"> <button type=\"button\" class=\"submit-button\" onclick=\"handlePOST('11', 0, 0)\">Toggle All Lights</button> </form>";
#else
String  DEVICE1_FORM = "]; let index1 = 0; for (let row = 1; row <= 4; row++) { document.write(\"<tr>\"); for (let col = 1; col <= 5; col++) { document.write(`<td><input type=\"number\" id=\"input0_${row}${col}\" name=\"input${row}_${col}\" value=\"${values1[index1]}\"></td>`); index1++;} document.write(\"</tr>\"); } </script> </tbody> </table> <br><hr class=\"stylish-line\"> <button type=\"button\" class=\"submit-button\" onclick=\"submitForm('10', 0)\">Assign Lights ID To Group1</button> <br><br> <button type=\"button\" class=\"submit-button\" onclick=\"handlePOST('11', 0, 0)\">Toggle All Lights In Group1</button> </form>";
#endif
#else
...
String TAB3_START_DALI ="<form action=\"/action\" method=\"post\" onsubmit=\"submitData(event, this, 0, 12)\"> <div class=\"form-group\"> <h3>Max Lights to Assign Address</h3> <button type=\"submit\" class=\"stylish-button\" style=\"width: 300px;\">Start Assign Dali Addresses</button> <input type=\"number\" id=\"max_addr\" name=\"max_addr\" value=\"";
String TAB3_START_DALI_CONTINUE="\" placeholder=\"Total Addr\" min=\"0\" max=\"63\" style=\"width: 120px;\"> </div> </form>";
String TAB3_SHOW_DALI ="<div> <form  method=\"get\" onsubmit=\"customSubmit(event)\" id=\"myFormGet\"> <div class=\"form-group\"> <button type=\"submit\" class=\"stylish-button\" style=\"width: 300px;\">Show All DALI Assigned IDs</button>  <div id=\"waitProgress\" style=\"display:none; text-align:center; margin:10px;\"> <span>Loading, please wait...</span> <div class=\"loading-spinner\" style=\"margin:auto\"></div> </div> <input class=\"hidden\" type=\"number\" id=\"query\" name=\"max_addr\" value=\"10\" placeholder=\"Get All\" min=\"0\" max=\"63\" style=\"width: 120px; display: none;\"> </div> </form> <div id=\"tableContainer\"></div> </div>";
#endif
String TAB3_END = "</div>";
...
/////////////SCRIPTS///////////
String SCRIPT_HANDLE_SUBMIT ="function handleSubmit(event, fxn) { event.preventDefault(); document.getElementById('waitProgress').style.display = 'block'; const query = document.getElementById('query').value; fetch(`/items?fxn=${encodeURIComponent(fxn)}`) .then(response => response.text()) .then(data => { document.getElementById('waitProgress').style.display = 'none'; try { const jsonObj = JSON.parse(data); let tableHTML = jsonToTable(jsonObj); console.log(\"Raw object:\", jsonObj); console.log(\"Pretty JSON:\\n\", JSON.stringify(jsonObj, null, 2)); document.getElementById('tableContainer').innerHTML = tableHTML; let numberOfItems = jsonObj.length; let index = 0; jsonObj.forEach(item => { let id_index = item.Id; updateValue1(index++, id_index); }); } catch (e) { document.getElementById('tableContainer').innerHTML = '<b>Invalid JSON data! </b>'; } }) .catch(error => { console.error('Error:', error); document.getElementById('waitProgress').style.display = 'none'; document.getElementById('tableContainer').innerHTML = '<b>Error loading data! </b>'; }); } document.getElementById('myFormGet').addEventListener('submit', handleSubmit)";
String SCRIPT_HANDLE_POST ="function handlePOST(_fxn, _index, _value){ const jsonData = { fxn: _fxn, index: _index, value: _value }; const xhr = new XMLHttpRequest(); xhr.open(\"POST\", \"/action\", true); xhr.setRequestHeader(\"Content-Type\", \"application/json\"); xhr.onreadystatechange = function() { if (xhr.readyState === XMLHttpRequest.DONE) { if (xhr.status === 200) { console.log(\"Request successful\"); } else { console.error(\"Request failed\"); } } }; xhr.send(JSON.stringify(jsonData)); }";

String SCRIPT_CUSTOM_SUBMIT ="function customSubmit(event) { event.preventDefault(); document.getElementById('customModal').style.display = 'flex'; document.getElementById('btnRead').onclick = null; document.getElementById('btnStartNew').onclick = null; document.getElementById('btnCancel').onclick = null; document.getElementById('btnRead').onclick = function() { document.getElementById('customModal').style.display = 'none'; handleSubmit(event, 10); }; document.getElementById('btnStartNew').onclick = function() { document.getElementById('customModal').style.display = 'none'; handleSubmit(event, 12); }; document.getElementById('btnCancel').onclick = function() { document.getElementById('customModal').style.display = 'none'; }; return false; }";
String SCRIPT_CUSTOM_MODAL ="<div id=\"customModal\" style=\"display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.4); z-index:1000; align-items:center; justify-content:center;\"> <div style=\"background:#fff; padding:30px 20px; border-radius:8px; min-width:300px; text-align:center; box-shadow:0 2px 12px rgba(0,0,0,0.2);\"> <h3>What would you like to do?</h3> <button id=\"btnRead\" class=\"stylish-button\" style=\"margin:10px;\">Read Saved Addresses</button> <button id=\"btnStartNew\" class=\"stylish-button\" style=\"margin:10px;\">Read Physical Addresses</button> <button id=\"btnCancel\" class=\"stylish-button\" style=\"margin:10px;\">Cancel</button> </div> </div>";
String SCRIPT_SHOW_TABLE ="function jsonToTable(jsonObj) { let table = `<table border=\"1\" cellpadding=\"8\" style=\"border-collapse: collapse; width: 100%;\"> <tr> <th>ID</th> <th>State</th> <th>Toggle</th> </tr>`; jsonObj.forEach(item => { const checked = item.state ? 'checked' : ''; table += `<tr> <td>${item.Id}</td> <td>${item.state ? 'ON' : 'OFF'}</td> <td> <label class=\"switch\"> <input type=\"checkbox\" data-id=\"${item.Id}\" ${checked} onchange=\"toggleState(this)\"> <span class=\"slider\"></span> </label> </td> </tr>`; }); table += '</table>'; return table; }";

String SCRIPT_TOGGLE_STATE ="function toggleState(checkbox) { const id = checkbox.getAttribute('data-id'); const newState = checkbox.checked ? 1 : 0; fetch(`/items?fxn=11&id=${id}&state=${newState}`, { method: 'GET' }) .then(res => res.text()) .then(msg => console.log('Toggle response:', msg)); }";

String SCRIPT_TABLE_STYLE ="const style = document.createElement('style'); style.textContent = `.switch { position: relative; display: inline-block; width: 40px; height: 22px; } .switch input { opacity: 0; width: 0; height: 0; } .slider { position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background-color: #ccc; transition: .4s; border-radius: 22px; } .slider:before { position: absolute; content: \"\"; height: 18px; width: 18px; left: 2px; bottom: 2px; background-color: white; transition: .4s; border-radius: 50%; } input:checked + .slider { background-color: #2196F3; } input:checked + .slider:before { transform: translateX(18px); }`; document.head.appendChild(style);";
...
String SCRIPT_SET_INTENSITY ="document.addEventListener('DOMContentLoaded', function() { const slider = document.getElementById('stepSlider1'); const sliders_intensity = document.querySelectorAll(\".slider-container-1\"); sliders_intensity.forEach((sliderContainer, _index) => { const slider1 = sliderContainer.querySelector(\".slider-1\"); const thumbLabel = sliderContainer.querySelector(\".thumb-label\"); slider1.addEventListener(\"input\", function() { const _value = parseInt(this.value); const thumb = this.querySelector(\".slider-1::-webkit-slider-thumb\"); thumbLabel.textContent = _value; thumbLabel.style.display = \"block\"; thumbLabel.style.left = `calc(${_value * 100 / parseInt(this.max)}% - ${thumbLabel.offsetWidth / 2}px)`; handlePOST('17', _index, _value); }); }); slider.dispatchEvent(new Event('change')); });";
    String SCRIPT_UPDATE_INTENSITY ="function updateSlider(newValue) { const slider = document.getElementById('stepSlider1'); slider.value = newValue; slider.dispatchEvent(new Event('change')); }";
String SCRIPT_UPDATE_INTENSITY_VAL_START ="updateSlider(";
String SCRIPT_UPDATE_INTENSITY_VAL_SET ="30";
String SCRIPT_UPDATE_INTENSITY_VAL_END =");";
...
String SCRIPT_SCENES_SELECTOR ="const maxButtons = 6; function selectScene(_index, _scene) { const buttons = document.querySelectorAll('.button-selector'); buttons.forEach(button => { const num = scene.toString(); if (button.id === `scene${index+1}_${scene}`) { button.classList.add('selected'); } else { button.classList.remove('selected'); } }); handlePOST('18', _index, _scene); }";
    ...
    String SCRIPT_SUBMIT_FORM ="function submitForm(fxn, formIndex) { let inputData = { \"fxn\": fxn, \"index\": formIndex, \"values\": [] }; for (let row = 1; row <= 4; row++) { for (let col = 1; col <= 5; col++) { let inputId = `input${formIndex}_${row}${col}`; let inputValue = document.getElementById(inputId).value; inputData.values.push(parseInt(inputValue)); } }  let jsonData = JSON.stringify(inputData); fetch('/action', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: jsonData }) .then(response => response.json()) .then(data => { console.log('Success:', data); }) .catch((error) => { console.error('Error:', error); }); }";
    #else
    String SCRIPT_SUBMIT_FORM ="function submitForm(fxn, formIndex) { let inputData = { \"fxn\": fxn, \"index\": formIndex, \"values\": [] }; for (let row = 1; row <= 2; row++) { for (let col = 1; col <= 5; col++) { let inputId = `input${formIndex}_${row}${col}`; let inputValue = document.getElementById(inputId).value; inputData.values.push(parseInt(inputValue)); } }  let jsonData = JSON.stringify(inputData); fetch('/action', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: jsonData }) .then(response => response.json()) .then(data => { console.log('Success:', data); }) .catch((error) => { console.error('Error:', error); }); }";
#endif
String SCRIPT_FORM_ARRAY_VALUES ="const newValues = [ {\"input1_1\":1, \"input1_2\":2, \"input1_3\":3, \"input1_4\":4, \"input1_5\":5, \"input2_1\":0, \"input2_2\":0, \"input2_3\":0, \"input2_4\":0, \"input2_5\":0, \"input3_1\":0, \"input3_2\":0, \"input3_3\":0, \"input3_4\":0, \"input3_5\":0, \"input4_1\":0, \"input4_2\":0, \"input4_3\":0, \"input4_4\":0, \"input4_5\":0}, {\"input1_1\":1, \"input1_2\":2, \"input1_3\":3, \"input1_4\":4, \"input1_5\":5, \"input2_1\":0, \"input2_2\":0, \"input2_3\":0, \"input2_4\":0, \"input2_5\":0, \"input3_1\":0, \"input3_2\":0, \"input3_3\":0, \"input3_4\":0, \"input3_5\":0, \"input4_1\":0, \"input4_2\":0, \"input4_3\":0, \"input4_4\":0, \"input4_5\":0},{\"input1_1\":1, \"input1_2\":2, \"input1_3\":3, \"input1_4\":4, \"input1_5\":5, \"input2_1\":0, \"input2_2\":0, \"input2_3\":0, \"input2_4\":0, \"input2_5\":0, \"input3_1\":0, \"input3_2\":0, \"input3_3\":0, \"input3_4\":0, \"input3_5\":0, \"input4_1\":0, \"input4_2\":0, \"input4_3\":0, \"input4_4\":0, \"input4_5\":0}, {\"input1_1\":1, \"input1_2\":2, \"input1_3\":3, \"input1_4\":4, \"input1_5\":5, \"input2_1\":0, \"input2_2\":0, \"input2_3\":0, \"input2_4\":0, \"input2_5\":0, \"input3_1\":0, \"input3_2\":0, \"input3_3\":0, \"input3_4\":0, \"input3_5\":0, \"input4_1\":0, \"input4_2\":0, \"input4_3\":0, \"input4_4\":0, \"input4_5\":0} ];";
    String SCRIPT_ARRAY_START = "const newValues =";
String SCRIPT_UPDATE_IP_FIELDS ="function updateInputFields(formIndex, values) { for (let row = 1; row <= 4; row++) { for (let col = 1; col <= 5; col++) { let inputId = `input${formIndex}_${row}${col}`; let valueKey = `input${row}_${col}`; document.getElementById(inputId).value = values[valueKey] || 0; } } }";
...
String SCRIPT_SUBMIT_DATA ="function submitData(event, formElement, index, fxn) { event.preventDefault(); document.getElementById('waitProgress').style.display = 'block'; let formData = new FormData(formElement); formData.append('fxn', fxn); let jsonData = {}; for (let [key, value] of formData.entries()) { jsonData[key] = value; } var json_data = JSON.stringify(jsonData); console.log(json_data); var xhr = new XMLHttpRequest(); xhr.open(\"POST\", \"/action\", true); xhr.setRequestHeader(\"Content-Type\", \"application/json\"); xhr.onreadystatechange = function() { if (xhr.readyState === XMLHttpRequest.DONE) { document.getElementById('waitProgress').style.display = 'none'; if (xhr.status === 200) { console.log(\"Request successful\"); } else { console.error(\"Request failed\"); } } }; xhr.send(json_data); }";
...
String BODY_DALI_SCENES ="<div class=\"card\"> <h2 id=\"mainHeader\">DALI Scene Control</h2> <p>Set Group, Scene IDs (0–15) or Dali IDs (0–63).</p> <p>Choose Switch Control Type, and submit to your device.</p><br> <form id=\"sceneForm\" autocomplete=\"off\"> <label for=\"control_type\" style=\"color: #3b82f6;\">Switch Control Type:</label> <select id=\"control_type\" required> <option>Individual Control</option> <option>Group Control</option> <option>Scene Control</option> </select> <div id=\"sceneControlDiv\" style=\"display:none; margin-top:1em; color: #3b82f6;\"> <label for=\"control_type1\">Scene Control Type:</label> <select id=\"control_type1\" required> <option>Broadcast Control</option> <option>Group Control</option> <option>Individual Control</option> </select> </div> <br> <br> <br> <label for=\"group_id\" id=\"groupIdLabel\" style=\"color: #3b82f6;\">Group ID (0–15):</label> <input type=\"number\" id=\"group_id\" min=\"0\" max=\"15\" value=\"0\"> <p>Long Press each ID fileds to set DALI Ids</p> <br> <div class=\"row\"> <div> <label for=\"scene1\" id=\"scene1Label\" style=\"color: #3b82f6;\">Scene ID 1:</label> <input type=\"number\" id=\"scene1\" min=\"0\" max=\"15\" required> </div> <div> <label for=\"scene2\" id=\"scene2Label\" style=\"color: #3b82f6;\">Scene ID 2:</label> <input type=\"number\" id=\"scene2\" min=\"0\" max=\"15\" required> </div> </div> <div class=\"row\"> <div> <label for=\"scene3\" id=\"scene3Label\" style=\"color: #3b82f6;\">Scene ID 3:</label> <input type=\"number\" id=\"scene3\" min=\"0\" max=\"15\" required> </div> <div> <label for=\"scene4\" id=\"scene4Label\" style=\"color: #3b82f6;\">Scene ID 4:</label> <input type=\"number\" id=\"scene4\" min=\"0\" max=\"15\" required> </div> </div> <br> <button class=\"stylish-button\" type=\"submit\">Save Changes</button> </form> <pre id=\"result\"></pre> </div>";
String BODY_DALI_OVERLAY_SCENES ="<div class=\"overlay\" id=\"overlay\"></div> <div class=\"popup\" id=\"popup\"> <h3>Select IDs</h3> <div id=\"toggleGrid\"></div> <div style=\"margin-top: 15px;\"> <button style=\"padding: 10px 20px;\" onclick=\"submitSelection()\">Submit</button> <button style=\"padding: 10px 20px;\" onclick=\"closePopup()\">Cancel</button> </div> </div>";

String SCRIPT_DALI_SCENES ="function updateHeader(newText) { document.getElementById('mainHeader').textContent = newText; } function updateTitle(newTitle) { document.title = newTitle; updateHeader(newTitle); } function setInputRange(min, max) { let input1 = document.getElementById('scene1'); input1.min = min; input1.max = max; let input2 = document.getElementById('scene2'); input2.min = min; input2.max = max; let input3 = document.getElementById('scene3'); input3.min = min; input3.max = max; let input4 = document.getElementById('scene4'); input4.min = min; input4.max = max; } function updateGroupIdVisibility(show) { var groupIdLabel = document.getElementById('groupIdLabel'); var groupIdInput = document.getElementById('group_id'); groupIdLabel.style.display = show ? '' : 'none'; groupIdInput.style.display = show ? '' : 'none'; } document.getElementById('control_type').addEventListener('change', function() { var val = this.value; var groupIdLabel = document.getElementById('groupIdLabel'); var groupIdInput = document.getElementById('group_id'); var sceneLabels = [ document.getElementById('scene1Label'), document.getElementById('scene2Label'), document.getElementById('scene3Label'), document.getElementById('scene4Label') ];   if (val === 'Individual Control') { groupIdLabel.style.display = 'none'; groupIdInput.style.display = 'none'; sceneLabels[0].innerText = 'Dali ID 1:'; sceneLabels[1].innerText = 'Dali ID 2:'; sceneLabels[2].innerText = 'Dali ID 3:'; sceneLabels[3].innerText = 'Dali ID 4:'; setInputRange(0, 63); updateTitle(\"DALI Id's Control Switch\"); updateGroupIdVisibility(false); } else if (val === 'Group Control' || val === 'Broadcast Control') { groupIdLabel.style.display = 'none'; groupIdInput.style.display = 'none'; sceneLabels[0].innerText = 'Group ID 1:'; sceneLabels[1].innerText = 'Group ID 2:'; sceneLabels[2].innerText = 'Group ID 3:'; sceneLabels[3].innerText = 'Group ID 4:'; if (val === 'Group Control'){ updateTitle(\"DALI Group Control Switch\"); updateGroupIdVisibility(false); setInputRange(0, 15); }else{ updateTitle(\"DALI Broadcast Control Switch\"); setInputRange(0, 15); updateGroupIdVisibility(false); } } else if (val === 'Scene Control') { groupIdLabel.style.display = ''; groupIdInput.style.display = ''; sceneLabels[0].innerText = 'Scene ID 1:'; sceneLabels[1].innerText = 'Scene ID 2:'; sceneLabels[2].innerText = 'Scene ID 3:'; sceneLabels[3].innerText = 'Scene ID 4:'; setInputRange(0, 15); } var sceneDiv = document.getElementById('sceneControlDiv'); if (this.value === 'Scene Control') { sceneDiv.style.display = 'block'; var val = document.getElementById('control_type1').value; if (val === 'Broadcast Control') { updateGroupIdVisibility(false); updateTitle(\"DALI Scene Control Switch\"); setInputRange(0, 15); }else if(val === 'Individual Control') { updateGroupIdVisibility(false); updateTitle(\"DALI Scene Control Switch\"); setInputRange(0, 63); } else if (val === 'Group Control') { updateGroupIdVisibility(true); updateTitle(\"DALI Scene Control Switch (group required)\"); setInputRange(0, 15); } } else { sceneDiv.style.display = 'none'; } }); document.getElementById('control_type1').addEventListener('change', function() { var val = this.value; if (document.getElementById('sceneControlDiv').style.display !== 'none') { if (val === 'Broadcast Control' || val === 'Individual Control') { updateGroupIdVisibility(false); } else if (val === 'Group Control') { updateGroupIdVisibility(true); } } }); document.getElementById('sceneForm').onsubmit = async function(e) { e.preventDefault(); const data = { fxn: '40', group_id: parseInt(document.getElementById('group_id').value), scene_ids: [ parseInt(document.getElementById('scene1').value), parseInt(document.getElementById('scene2').value), parseInt(document.getElementById('scene3').value), parseInt(document.getElementById('scene4').value) ], control_type:document.getElementById('control_type').selectedIndex, scn_ctrl_type:document.getElementById('control_type1').selectedIndex }; const res = await fetch('/action', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(data) }); }; document.addEventListener('DOMContentLoaded', function() { fetch('/items?fxn=15') .then(response => response.json()) .then(data => { document.getElementById('group_id').value = data.group_id; document.getElementById('scene1').value = data.scene_ids[0]; document.getElementById('scene2').value = data.scene_ids[1]; document.getElementById('scene3').value = data.scene_ids[2]; document.getElementById('scene4').value = data.scene_ids[3]; document.getElementById('control_type').value = data.control_type; document.getElementById('control_type1').value = data.scn_ctrl_type; if (document.getElementById('control_type').value == 'Scene Control' || document.getElementById('control_type').value == 'Individual Control' || document.getElementById('control_type').value == 'Broadcast Control') { updateGroupIdVisibility(false); }else{ updateGroupIdVisibility(true); } if (document.getElementById('control_type').value == 'Scene Control'){ document.getElementById('sceneControlDiv').style.display = 'block'; }else{ document.getElementById('sceneControlDiv').style.display = 'none'; }}); fetch('/items?fxn=16') .then(response => response.json()) .then(data => { if (Array.isArray(data) && data.length === 4) { popupSelections = data.map(arr => Array.isArray(arr) ? arr : []); } console.log('Loaded popupSelections:', popupSelections); }) .catch(err => { console.error('Failed to load popupSelections:', err); });});";
String SCRIPT_DALI_SCENES_POPUP ="let currentInput = null; let popupSelections = [[], [], [], []]; (function initGrid() { let grid = ''; for (let i = 0; i < 64; i++) { grid += `<button class=\"toggle-btn\" id=\"btn${i}\" onclick=\"toggleBtn(${i})\">${i}</button>`; if (i % 8 === 0) grid += '<br>'; } document.getElementById('toggleGrid').innerHTML = grid; })(); ['scene1','scene2','scene3','scene4'].forEach((id, idx) => { let el = document.getElementById(id); let pressTimer; function tryShowPopup() { const ctrlType = document.getElementById('control_type').value; if (ctrlType === 'Individual Control' || ctrlType === 'Group Control') { openPopup(idx); }else if (ctrlType === 'Scene Control') { const ctrlType1 = document.getElementById('control_type1').value; if (ctrlType1 === 'Individual Control') { openPopup(idx); } } } function sendClickIndex(idx, id) { const payload = { fxn: '42', index: idx, id: id }; fetch('/action', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }) .then(res => res.text()) .then(data => { }) .catch(err => { alert('Error sending data: ' + err); }); } el.addEventListener('mousedown', () => pressTimer = setTimeout(tryShowPopup, 600)); el.addEventListener('mouseup', () => clearTimeout(pressTimer)); el.addEventListener('mouseleave', () => clearTimeout(pressTimer)); el.addEventListener('touchstart', () => pressTimer = setTimeout(tryShowPopup, 600)); el.addEventListener('touchend', () => clearTimeout(pressTimer)); el.addEventListener('click', function() { sendClickIndex(idx, id); }); }); function openPopup(inputIdx) { currentInput = inputIdx; let selected = popupSelections[inputIdx] || []; for (let i = 0; i < 64; i++) { const btn = document.getElementById('btn' + i); if (selected.includes(i)) { btn.classList.add('selected'); } else { btn.classList.remove('selected'); } } document.getElementById('popup').style.display = 'block'; document.getElementById('overlay').style.display = 'block'; } function toggleBtn(id) { let selected = popupSelections[currentInput] || []; const index = selected.indexOf(id); if (index === -1) { selected.push(id); } else { selected.splice(index, 1); } popupSelections[currentInput] = selected; const btn = document.getElementById(`btn${id}`); btn.classList.toggle('selected'); } function submitSelection() { if (currentInput === null) return; const selected = popupSelections[currentInput].slice().sort((a, b) => a - b); const payload = { fxn: '41', input_id: currentInput, selected_ids: selected }; fetch('/action', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }) .then(res => res.text()) .then(data => { closePopup(); }) .catch(err => { alert('Error sending data: ' + err); closePopup(); }); } function closePopup() { document.getElementById('popup').style.display = 'none'; document.getElementById('overlay').style.display = 'none'; }";
#endif
#endif
...
#if(USE_NUOS_ZB_DEVICE_TYPE == DEVICE_SCENE_SWITCH)
strcat(webpage, STYLE_SCENE_SWITCH);
strcat(webpage, STYLE_SCENE_SWITCH_POPUP);
#endif
#elif(USE_NUOS_ZB_DEVICE_TYPE == DEVICE_CCT_DALI || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_RGB_DALI || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_RGB_DMX || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_RGB_CUSTOM || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_GROUP_DALI || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_CCT_DALI_CUSTOM)
#if(USE_NUOS_ZB_DEVICE_TYPE == DEVICE_CCT_DALI  || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_CCT_DALI_CUSTOM)
strcat(webpage, STYLE_COLOR_TEMP);
...
String scene_store_http_data = "var savedGroupId = %d;var savedSceneIds = [%d,%d,%d,%d]; var savedControlType = '%s'; var savedControlType1 = '%s'; document.addEventListener('DOMContentLoaded',function(){document.getElementById('group_id').value = savedGroupId;document.getElementById('scene1').value = savedSceneIds[0];document.getElementById('scene2').value = savedSceneIds[1];document.getElementById('scene3').value = savedSceneIds[2];document.getElementById('scene4').value = savedSceneIds[3]; document.getElementById('control_type').value = savedControlType; document.getElementById('control_type1').value = savedControlType1;});";
...
strcat(webpage, SCRIPT_SUBMIT_DATA);
       // strcat(webpage, SCRIPT_OPEN_TAB2_BY_USER);
    #elif(USE_NUOS_ZB_DEVICE_TYPE == DEVICE_RGB_DALI || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_CCT_DALI || USE_NUOS_ZB_DEVICE_TYPE == DEVICE_CCT_CUSTOM)
strcat(webpage, SCRIPT_ARRAY_START);
strcat(webpage, convert_to_json());
strcat(webpage, ";");
strcat(webpage, SCRIPT_UPDATE_IP_FIELDS);
...
snprintf(&webpage[strlen(webpage)], sizeof(webpage),
scene_store_http_data,
scene_group_switch_info.group_id,
...
switch_ctrl_type[scene_group_switch_info.control_type],
scene_ctrl_type[scene_group_switch_info.scn_ctrl_type]);

strcat(webpage, SCRIPT_DALI_SCENES);
strcat(webpage, SCRIPT_DALI_SCENES_POPUP);
...
strcat(webpage, SCRIPT_CUSTOM_SUBMIT);

strcat(webpage, SCRIPT_SHOW_TABLE);
strcat(webpage, SCRIPT_TOGGLE_STATE);
strcat(webpage, SCRIPT_TABLE_STYLE);
strcat(webpage, SCRIPT_HANDLE_SUBMIT);
#endif
#endif
...
strcat(webpage, SCRIPT_END);
#if(USE_NUOS_ZB_DEVICE_TYPE == DEVICE_GROUP_DALI)
    strcat(webpage, SCRIPT_CUSTOM_MODAL);
#endif
strcat(webpage, BODY_END);
strcat(webpage, HTML_END);
    printf("\n\n");
...
prepare_html_start_wifi_string();
    ...
    #if(USE_NUOS_ZB_DEVICE_TYPE == DEVICE_SCENE_DALI)
    strcat(webpage, BODY_DALI_SCENES);
strcat(webpage, BODY_DALI_OVERLAY_SCENES);
#else
prepare_html_tab2_string();
prepare_html_tab3_string();
prepare_html_tab4_string();
