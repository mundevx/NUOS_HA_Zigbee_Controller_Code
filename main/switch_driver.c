...
// #define DEBOUNCE_US                 30000
// #define LONG_PRESS_US               700000
// #define LONG_HOLD_REPEAT_US         150000
// #define MULTI_CLICK_GAP_US          250000
// #define COMBO_LONG_PRESS_US         800000
...
//     if (match_6(0,1,0,1,0,1)) {
//         ESP_LOGI(TAG, "PATTERN DETECTED: 1,2,1,2,1,2");
//         printf("Pattern 1,2,1,2,1,2 detected\n");
//         reset_click_pattern();
//     }
//     else if (match_6(0,2,0,2,0,2)) {
//         ESP_LOGI(TAG, "PATTERN DETECTED: 1,3,1,3,1,3");
//         printf("Pattern 1,3,1,3,1,3 detected\n");
//         reset_click_pattern();
//     }
//     else if (match_6(0,2,2,0,0,2)) {
//         ESP_LOGI(TAG, "PATTERN DETECTED: 1,3,3,1,1,3");
//         printf("Pattern 1,3,3,1,1,3 detected\n");
//         reset_click_pattern();
//         if(ready_commisioning_flag){
//             for(int i=0; i<50; i++) {
//                 esp_err_t status = esp_zb_zcl_scenes_table_clear_by_index(i);
...
//         }
//     }
//     else if (match_6(2,0,0,2,2,0)) {
//         ESP_LOGI(TAG, "PATTERN DETECTED: 3,1,1,3,3,1");
//         printf("Pattern 3,1,1,3,3,1 detected\n");
//         reset_click_pattern();
//     }
// }
...
//         ESP_LOGI(TAG, "SWITCH_EVENT_COMBO_LONG_PRESS mask=0x%02" PRIx32 " func=0x%02x",
//                  combo_mask, out.func);
//         if(combo_mask == 0x05){ //button 1 & 3 are pressed
//             printf("Zigbee Comissioning Buttons Detected!!\n");
//         }else if(combo_mask == 0x0A){ // button 2 and 4 are pressed
//             printf("WiFi Webserver Buttons Detected!!\n");
//         }
//         if (out.func != SWITCH_NOTHING_CONTROL) {
//             func_ptr(&out);
...
//             out.func = switch_func_pair[idx].single_func;
//             out.keypressed = SINGLE_PRESS;
//             // detect_button_pattern(out.id, ts_us);
//             break;
...
//     bool new_level = evt->level ? true : false;   // pull-up: 1=released, 0=pressed
//     if (new_level == b->stable_level) {
//         return;
//     }
...
//         //ESP_LOGI(TAG, "BTN[%d] pressed pin=%d", b->button_id, b->pin);
//     } else {
//         b->pressed = false;
//         b->release_time_us = evt->isr_time_us;
//         b->last_hold_log_us = 0;

//         //ESP_LOGI(TAG, "BTN[%d] released pin=%d", b->button_id, b->pin);
...
//     // 1. First: check if active combo is still valid
//     if (g_combo_active) {
//         if (cur_mask != g_active_combo_mask || cur_mask == 0) {
//             ESP_LOGI(TAG, "Combo released/changed: old=0x%02" PRIx32 " new=0x%02" PRIx32,
//                     g_active_combo_mask, cur_mask);
...
//             if ((now - g_combo_last_log_us) >= COMBO_HOLD_REPEAT_US) {
//                 g_combo_last_log_us = now;
//                 log_combo_hold_message(g_active_combo_mask);
//                 // if (g_active_combo_mask == 0x05) {   // button 1 & 3 in your naming
//                 //     printf("Zigbee Comissioning Buttons Detected!!\n");
//                 // } else if (g_active_combo_mask == 0x0A) {   // button 2 & 4
//                 //     printf("WiFi Webserver Buttons Detected!!\n");
//                 // } else {
//                 //     printf("Combo still pressed, mask=0x%02" PRIx32 "\n", g_active_combo_mask);
//                 // }
//             }
//             return;
//         }
...
//     // 2. Then: detect new combo long press
//     if (!g_combo_long_reported && pressed_count() >= 2) {
//         int64_t oldest_press = now;
//         uint32_t combo_mask = cur_mask;
...
//     // 3. Then: normal single-button long press and repeat
//     for (int i = 0; i < switch_num; i++) {
//         button_state_t *b = &g_btn[i];
...
//         // ESP_LOGI(TAG, "Register button id=%d pin=%d events=0x%08" PRIx32,
//         //          g_btn[i].button_id, g_btn[i].pin, button_func_pair[i].enabled_events);
//     }
