...
#include "ha/esp_zigbee_ha_standard.h"
    ...
    volatile bool pir_motion_disable_flag = false;
    int32_t pir_motion_disable_timeout_counts = 0;
    int32_t pir_motion_disable_timeout_value = 60; //1 minute
    int32_t motion_auto_enable_flag = 0;
    extern void nuos_set_zigbee_attribute(uint8_t index);


    uint8_t comm_counts = 0;
    static bool lb_state = 0;

static void sensor_task(void *pvParameters) {
    //static TickType_t last_detection_time = 0;
    //const TickType_t detection_interval = pdMS_TO_TICKS(30000); // 5 seconds
    //const TickType_t send_2_zb_detection_interval = pdMS_TO_TICKS(60000); // 60 seconds
    while (1) {
        ...
        if(!start_commissioning) {
            ...
            if(!pir_motion_disable_flag){
                if (sensor_detected_flag) { //&& (current_time - last_detection_time >= detection_interval)) {
                ...
                printf("timeout_counts:%ld\n", pir_motion_disable_timeout_counts);
                if(motion_auto_enable_flag>0){
                    ...
                    if(pir_motion_disable_timeout_counts++ >= 10){
    ...
    void nuos_toggle_leds(uint8_t index){
        lb_state = !lb_state;       
        nuos_on_off_led(index, lb_state);
    }
    ...
    void nuos_set_state_touch_leds(bool _state){
